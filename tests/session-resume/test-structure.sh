...
assert_file_exists "skills/session-resume/SKILL.md" "skill file exists"

# 2. Frontmatter opens with `---` (we use a YAML-aware check).
assert_python "
import re
text = open('__PROJECT_ROOT__/skills/session-resume/SKILL.md').read()
m = re.match(r'^---\n(.*?)\n---', text, re.DOTALL)
...
assert_grep "skills/session-resume/SKILL.md" '^stage: any' "stage is \"any\" (not stage 1–4)"

# 4. Body has Step 1 (find active epic), Step 2 (read state), Step 3 (manifest), Step 4 (decision), Step 5 (output).
assert_grep "skills/session-resume/SKILL.md" '^### Step 1\. ' "procedure: Step 1 exists"
assert_grep "skills/session-resume/SKILL.md" '^### Step 2\. ' "procedure: Step 2 exists"
assert_grep "skills/session-resume/SKILL.md" '^### Step 3\. ' "procedure: Step 3 exists"
assert_grep "skills/session-resume/SKILL.md" '^### Step 4\. ' "procedure: Step 4 exists"
assert_grep "skills/session-resume/SKILL.md" '^### Step 5\. ' "procedure: Step 5 exists"

# 5. Body has Anti-patterns section.
assert_grep "skills/session-resume/SKILL.md" '^## Anti-patterns' "anti-patterns section exists"

# 6. Body has Claim/Verify section.
assert_grep "skills/session-resume/SKILL.md" '^## Claim / Verify' "claim/verify section exists"

# 7. Source-of-truth block references the manifest.
assert_grep "skills/session-resume/SKILL.md" 'Source of truth' "manifest is declared as source of truth"
